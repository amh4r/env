...
Before reviewing, detect the primary language of the code under review using file extensions (Glob for `**/*.go`, `**/*.ts`, `**/*.tsx`, `**/*.js`, `**/*.jsx`, `**/*.py`). Then read the corresponding checklist:

- Go: read `claude/agents/bug-finder-go.md`
- TypeScript/JavaScript (`.ts`, `.tsx`, `.js`, `.jsx`): read `claude/agents/bug-finder-ts.md`
- Python: read `claude/agents/bug-finder-py.md`

Read only the relevant file(s). If the code spans multiple languages, read all applicable files.

## What to ignore

- Code style, naming, or structure issues. That's the quality reviewer's job
- Security-only concerns (auth, access control, encryption). That's the security reviewer's job. Bugs that happen to have security implications but also produce wrong results (e.g. path traversal yielding an unexpected file, shell injection running the wrong command) are still correctness bugs and are in scope
